...
public class ControllerServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)	throws ServletException, IOException {
		...
		// 하드 코딩이고, 패스워드 admin으로 하면, true 반환 메서드.
		boolean status = bean.validate();

		// 간단한 유효성 체크를 통과한 후 부에 값을 전달하는 역할.
		if (status) {
			// mvc_success.jsp 뷰 페이지를 rd라는 객체에 담아서 전달하는 역할. 뷰로 전달한다. (request, response) 두개의 객체를 전달합니다.
			// 그러면 뷰에서 해당 객체에 담겨진 데이터를 꺼내서 사용하는 형식.
			RequestDispatcher rd = request.getRequestDispatcher("mvc_success.jsp");
			// 결과
			// 웹브라우저는 해당 뷰페이지의 내용만 불러오고, 주소는 변경되지 않는다.
